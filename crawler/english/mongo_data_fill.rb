...
begin
  @coll_bible.insert({ :_id => @bible_id.to_s, :name => index['bible'], :language => 'english', :url => index['url'],
  ...
  index['books'].each do |fbook|
    ...
    book['chapters'].each do |chapter, hash|
      ...
      hash['verses'].each do |number, verse|
        ...
        text = text.gsub(/'/ ,"''").gsub("\\", '\\\\\\\\')
        @coll_verse.insert({ :_id => @verse_id.to_s, :number => number, :text => text, :chapter_id => @chapter_id.to_s })
      end
      ...
      hash['annotations'].each do |number, annotation|
        ...
        @coll_annotation.insert({ :_id => @annotation_id.to_s, :number => number, :text => text, :chapter_id => @chapter_id.to_s })
      end
    end
  end
rescue Exception => ex
