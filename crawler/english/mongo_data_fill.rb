...
begin
  @coll_bible.insert({ :_id => @bible_id, :name => index['bible'], :language => 'english', :url => index['url'],
  ...
  index['books'].each do |fbook|
    ...
    book['chapters'].each do |chapter, hash|
      ...
      hash['verses'].each do |number, verse|
        ...
        text = text.gsub(/'/ ,"''").gsub("\\", '\\\\\\\\')
        @coll_verse.insert({ :_id => @verse_id, :number => number, :text => text, :chapter_fk => @chapter_id })
      end
      ...
      hash['annotations'].each do |number, annotation|
        ...
        @coll_annotation.insert({ :_id => @annotation_id, :number => number, :text => text, :chapter_fk => @chapter_id })
      end
    end
  end
rescue Exception => ex
